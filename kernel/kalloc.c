...
kfree(void *pa)
{
  ...
  push_off();
  int cpu_id=cpuid();
  acquire(&(kemArray[cpu_id].lock));
  r->next = kemArray[cpu_id].freelist;
  kemArray[cpu_id].freelist = r;
  release(&(kemArray[cpu_id].lock));
  pop_off();
}
...
kalloc(void)
{
  ...
  int cpu_id=cpuid();
  acquire(&(kemArray[cpu_id].lock));
  r = kemArray[cpu_id].freelist;
  if(r)
  {
    kemArray[cpu_id].freelist=r->next;
  }
  else
  {
